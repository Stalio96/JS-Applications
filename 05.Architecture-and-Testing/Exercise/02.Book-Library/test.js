...
describe('Tests', async function () {
    ...
    it('edit books', async () => {
        ...
        await page.fill('form#editForm >> input[name="title"]', 'Ivan');
        await page.fill('form#editForm >> input[name="author"]', 'Dragan');
        ...
        const data = JSON.parse(request.postData());
        expect(data.title).to.equal('Ivan');
        expect(data.author).to.equal('Dragan');
    });

    it.only('delete books', async () => {
        await page.goto('http://localhost:5500');
        ...
        const [request] = await Promise.all([
            ...
            page.click('tr >> text=Delete'),
            page.click('text=Ok')
        ]);

        //await page.click('text=Ok');

        expect(request.method()).to.equal(request.method('DELETE'));
    })
})