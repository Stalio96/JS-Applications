import { deleteBook, getBookById, getLikesByBook, getSpecificUser, likeBook } from '../api/data.js';
...
<section id="details-page" class="details">
    <div class="book-information">
        ...
        <div class="actions">
            ${bookPreview(book, isOwner, onDelete)}

            ${controls(showLikeBtn, onLike)}
...
const bookPreview = (book, isOwner, onDelete) => {
    if (isOwner) {
        return html`
...
const controls = (showLikeBtn, onLike) => {
    if (showLikeBtn) {
...
export async function detailsPage(ctx) {
    ...
    const [book, likes, hasLike] = await Promise.all([
        ...
        userData ? getSpecificUser(ctx.params.id, userData.id) : 0
    ]);
